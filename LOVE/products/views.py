...
def product_detail_view(request):
	...
	return render(request,"product2/detail2.html",context)
...
def product_create3_view(request):
	...
	if request.method == "POST":
		...
		if form2.is_valid():
			print(form2.cleaned_data) #經過valid合格後就是cleaned_data
...
def render_initial_data(request): # initial data和editing data都能儲存在實體頁面裡
	...
	obj = Product.objects.get(id=7) #一開始登入頁面輸入域中就會呈現product(id=1)的data
